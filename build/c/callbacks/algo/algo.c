LuaCEmbedResponse * create_hexa_decimal_string(LuaCEmbedTable *self,LuaCEmbed *args) {
    char *str = lua.args.get_str(args,0);
    if(lua.has_errors(args)){
        char *erro_msg = lua.get_error_message(args);
        return lua.response.send_error(erro_msg);
    }
    CTextStack *formmated = newCTextStack_string_empty();
    int size = strlen(str);
    for(int i = 0; i < size; i++) {
        char frmmated_buffer[20] = {0};
        sprintf(frmmated_buffer,"\\x%x",str[i]);
        CTextStack_format(formmated,"%s",frmmated_buffer);
    }
    LuaCEmbedResponse *respnse  = lua.response.send_str(formmated->rendered_text);
